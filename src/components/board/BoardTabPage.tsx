...
export default function BoardTabPage({ handleRow, className }: BoardTabProps) {
    ...
    return (
        <Menu as="div" className={`relative inline-block text-left ${className}`}>
            ...
            >
                <MenuItems className="absolute right-0 z-10 mt-2 w-16 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1">
                        {rows.map((el, i) => (
                            <MenuItem key={i}>
                                {({ focus }) => (
